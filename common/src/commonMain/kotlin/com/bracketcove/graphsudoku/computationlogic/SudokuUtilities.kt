...
internal fun rowsAreInvalid(puzzle: SudokuPuzzle): Boolean {
    ...
    (1..puzzle.boundary).forEach { row ->
        val nodeList = puzzle.graph[getHash(1, row)]!!
...
internal fun columnsAreInvalid(puzzle: SudokuPuzzle): Boolean {
    (1..puzzle.boundary).forEach { column ->
        val nodeList = puzzle.graph[getHash(column, 1)]!!
        (1..puzzle.boundary).forEach { value ->
            val occurrences = nodeList.filter { node ->
                node.x == value
            }.count { node ->
...
internal fun getNodesByColumn(graph: HashMap<Int,
        List<SudokuNode>>, x: Int): List<SudokuNode> {
...
internal fun getNodesByRow(graph: HashMap<Int,
        List<SudokuNode>>, y: Int): List<SudokuNode> {
    val edgeList = mutableListOf<SudokuNode>()
 ...
 *
 */
internal fun getNodesBySubgrid(graph: HashMap<Int,
        List<SudokuNode>>, x: Int, y: Int, boundary: Int): List<SudokuNode> {
    val edgeList = mutableListOf<SudokuNode>()
