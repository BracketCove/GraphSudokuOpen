 ...
 */
internal fun basicSolver(clique: List<SudokuNode>, boundary: Int): Boolean {
    ...
    while (solveable) {
        ...
        puzzle.graph.values.filter { adjList -> adjList.first().color == 0 }.forEach {
            ...
            if (basicSolver(it, puzzle.boundary)) solveable = true
                ...
                val superClique: List<SudokuNode> = getSuperClique(it.first(), puzzle)
 ...
 */
internal fun getSuperClique(first: SudokuNode, puzzle: SudokuPuzzle): List<SudokuNode> {
    val superClique = mutableListOf<SudokuNode>()
